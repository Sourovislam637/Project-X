...
async def _mirror_leech(client, message, isQbit=False, isLeech=False, sameDir=None, bulk=[]):
    ...
    elif isinstance(link, str) and not is_magnet(link) and link.split('?', 1)[0].lower().endswith('.m3u8'):
        # .m3u8 (HLS streaming) links can't be grabbed by aria2c directly - route through yt-dlp
        await delete_links(message)
        ydl = YoutubeDLHelper(listener)
        await ydl.add_download(link, f'{path}/', name, 'bv+ba/b', False, '')
